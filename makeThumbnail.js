...
exports.handler = async (event, context) => {
	...
	const event_time = decodeURIComponent(event.Records[0].s3.eventTime);
	...
	const getThumbnail = image => sharp(image)
		.jpeg({
			quality: 85,
			progress: true,
	...
	return await s3.getObject({Bucket: source_bucket, Key: source_filename}).promise()
		.then(object => getThumbnail(object.Body))
